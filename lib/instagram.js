...
// Parse Instagram's DASH manifest XML to extract all video representations and the audio URL.
...
function parseDashManifest(manifestXml) {
  const result = { videoReps: [], audioUrl: null };
  try {
    ...
    while ((m = repRegex.exec(manifestXml)) !== null) {
      ...
      if (mime.startsWith('audio/')) {
        ...
        result.videoReps.push({ url, bandwidth: bw });
      }
    }
    // Sort ascending by bandwidth so index 0 = lowest quality
    result.videoReps.sort((a, b) => a.bandwidth - b.bandwidth);
  } catch (_) {}
  return result;
}

const QUALITY_TIERS = ['240p', '360p', '480p', 'HD'];

function assignQualityLabels(videoReps) {
  const count = videoReps.length;
  return videoReps.map((rep, i) => {
    const tierIndex = QUALITY_TIERS.length - count + i;
    const quality = tierIndex >= 0 ? QUALITY_TIERS[tierIndex] : `Quality ${i + 1}`;
    const adRequired = i === count - 1; // highest bandwidth = HD = ad required
    return { ...rep, quality, adRequired };
  });
}
...
export async function getVideoFromUrl(url) {
  ...
  try {
    ...
    if (!interceptedVideoUrl) {
      ...
      if (dashMatch) {
        try {
          ...
          if (parsed.videoReps.length > 0) {
            dashVideoReps = assignQualityLabels(parsed.videoReps);
            dashAudioUrl = parsed.audioUrl;
            logger.info('Strategy 2: DASH manifest', {
              postId,
              representations: dashVideoReps.length,
              hasAudio: !!parsed.audioUrl,
              qualities: dashVideoReps.map(r => r.quality),
            });
          }
        } catch (e) {
    ...
    const dashTopVideoUrl = dashVideoReps.length > 0 ? dashVideoReps[dashVideoReps.length - 1].url : null;
    const videoUrl = interceptedVideoUrl || dashTopVideoUrl || ogData.video || domVideoUrl;
    ...
    logger.info('Strategy results', {
      ...
      s2_dash: dashTopVideoUrl ? dashTopVideoUrl.substring(0, 60) : null,
    ...
    let videos;
    if (interceptedVideoUrl) {
      videos = [{ url: videoUrl, audioUrl: null, quality: 'Original Quality', type: 'video/mp4', adRequired: false }];
    } else {
      videos = [
        ...dashVideoReps.map(rep => ({
          url: rep.url,
          audioUrl,
          quality: rep.quality,
          type: 'video/mp4',
          adRequired: rep.adRequired,
        })),
        ...(audioUrl ? [{ url: audioUrl, audioUrl: null, quality: 'Audio Track', type: 'audio/mp4', adRequired: false }] : []),
      ];
    }
