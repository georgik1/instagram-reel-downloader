...
export async function getVideoFromUrl(url) {
  ...
  try {
    ...
    // Build video options:
    // - Muxed (strategy 1a/1b): single "Original Quality" entry
    // - DASH: only the audio track is listed individually; video-only is intentionally omitted
    //   (the Combined download button handles video+audio together)
    let videos;
    if (interceptedVideoUrl) {
      videos = [{ url: videoUrl, quality: 'Original Quality', type: 'video/mp4' }];
    } else {
      videos = audioUrl ? [{ url: audioUrl, quality: 'Audio Track', type: 'audio/mp4' }] : [];
    }
